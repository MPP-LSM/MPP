...
program vsfm_celia1990
  ...
  use mesh_info            , only : nz, with_seepage_bc
  !
  implicit none
  !
  !
...
subroutine add_meshes()
  ...
  use mesh_info                 , only : nx, ny, nz, dx, dy, dz
  use mesh_info                 , only : slope_x, slope_y
...
subroutine add_conditions_to_goveqns()
  ...
  use MultiPhysicsProbConstants , only : CONN_VERTICAL
  use mesh_info                 , only : nx, ny, nz, dx, dy, dz
  use mesh_info                 , only : ncells_local, ncells_ghost, with_seepage_bc
  !
  ! !ARGUMENTS
  implicit none
  !
