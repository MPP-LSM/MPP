...
contains
  ...
  !------------------------------------------------------------------------
  subroutine setup_meshes(phtsyn)
    !
    implicit none
    !
    type(mpp_photosynthesis_type) :: phtsyn

    call add_meshes(phtsyn)

  end subroutine setup_meshes

  !------------------------------------------------------------------------
  subroutine add_meshes(phtsyn)
    ! !ARGUMENTS
    implicit none
    !
    type(mpp_photosynthesis_type) :: phtsyn

    call phtsyn%SetNumMeshes(1)
    call add_canopy_mesh(phtsyn)

  end subroutine add_meshes

  !------------------------------------------------------------------------
  subroutine add_canopy_mesh(phtsyn)
    !
    ...
    call phtsyn%AddMesh(PHTSYN_MESH, mesh)
    call mesh%Clean()
    deallocate(mesh)
...
end module photosynthesis_meshes
