...
contains
  ...
  !------------------------------------------------------------------------
  subroutine setup_meshes(longwave_mpp)
    !
    implicit none
    !
    type(mpp_longwave_type) :: longwave_mpp

    call add_meshes(longwave_mpp)

  end subroutine setup_meshes

  !------------------------------------------------------------------------
  subroutine add_meshes(longwave_mpp)
    ! !ARGUMENTS
    implicit none
    !
    type(mpp_longwave_type) :: longwave_mpp

    call longwave_mpp%SetNumMeshes(1)
    call add_canopy_mesh(longwave_mpp)

  end subroutine add_meshes

  !------------------------------------------------------------------------
  subroutine add_canopy_mesh(longwave_mpp)
    !
    ...
    call longwave_mpp%AddMesh(LONGWAVE_MESH, mesh)
    call mesh%Clean()
    deallocate(mesh)
