...
contains
  ...
  !------------------------------------------------------------------------
  subroutine add_conditions_to_goveqns(longwave_mpp)
    !
    ! !DESCRIPTION:
    !
    !
    ...
    call add_non_coupling_conditions_to_goveqns(longwave_mpp)

  end subroutine add_conditions_to_goveqns

  !------------------------------------------------------------------------
  subroutine add_non_coupling_conditions_to_goveqns(longwave_mpp)
    !
    ! !DESCRIPTION:
    !
    !
    ...
    allocate(conn_set)
    call MeshCreateConnectionSet(longwave_mpp%meshes(LONGWAVE_MESH), &
         nconn, id_up, id_dn, dist_up, dist_dn, area, itype, unit_vec, conn_set)

    call longwave_mpp%soe%AddConditionInGovEqn(LONGWAVE_GE, &
...
end module longwave_conditions
